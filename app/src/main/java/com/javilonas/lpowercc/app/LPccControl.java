...
public abstract class LPccControl {
	...
	protected String lpcc_command = "/res/init_uci.sh ";

	protected LPccValueChangedInterface vci; //interface to inform main activity about changed values
	...
	protected LPccControl(Activity activityIn)
	{
		activity = activityIn;
		context = activityIn;
		vci = (LPccValueChangedInterface) activityIn;
	}
	...
	{ 
		String command = lpcc_command + action + " " + valueFromUser;
	...
	public boolean getValueViaScript(boolean optimized) 
	{
		...
		if (this.canGetValueFromScript)
		{
			...
			if(optimized)
			{
				command = "`echo " + action + "|awk '{print \". /res/lpowercc/actions/\" $1,$1,$2,$3,$4,$5,$6,$7,$8}'`";
			}
			else
			{
				command = lpcc_command + action;
			}
			String response = Utils.executeRootCommandInThread(command);
			if(response != null)
			{
