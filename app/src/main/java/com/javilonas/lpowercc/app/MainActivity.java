...
public class MainActivity extends FragmentActivity implements ActionBar.TabListener, LPccValueChangedInterface, OnClickListener{
    ...
    private static ArrayList<LPccTab> lpccTabList = new ArrayList<LPccTab>();
	...
	private boolean getUserInterfaceConfigFromAssets() {
		Log.i(LOG_TAG, "Script LPowerCC no encontrado! Sin embargo está activado O_o");
		Log.i(LOG_TAG, "Obtener config xml de apk...");
		Boolean isOk = false;
		try 
		{     		
			InputStream is = getAssets().open("lpowercc.xml");
			isOk = parseUIFromXml(is);
		} 
		catch (IOException e) 
		{
			Log.i(LOG_TAG, "No existe config xml dentro de apk...");
			//e.printStackTrace();
		}
		return isOk;
	}
	...
	private boolean getUserInterfaceConfigFromScript() {
		Log.i(LOG_TAG, "Script LPowerCC encontrado.");
		Log.i(LOG_TAG, "Obtener config xml mediante script...");
		Boolean isOk = false;
		String response = Utils.executeRootCommandInThread("/res/init_uci.sh config");
		if (response != null && !response.equals(""))
		{
			Log.i(LOG_TAG, "Config xml extraido del script!");
	...
	{
		LPccExtrasTab it = new LPccExtrasTab(this, this);
	...
	private boolean isKernelSupportOk() {
		Boolean isOk = false;
		Log.i(LOG_TAG, "Buscando script LPowerCC...");
        File file = new File("/res/init_uci.sh");
        if (file.exists())
        {
            Log.i(LOG_TAG, "Kernel script(s) encontrado.");
        	if (file.canExecute())
        	{
        ...
        {
        	Log.e(LOG_TAG, "Kernel script(s) NO encontrado!");
        }
		return isOk;
	}
	...
	public boolean parseUIFromXml(InputStream is)
	{
		...
		try {
	    	...
	    	LPccTab tab = null;
	    	LPccPane pane = null;
	    	LPccSpinner spinner = null;
	    	LPccSeekBar seekbar = null;
	    	LPccCheckBox checkbox = null;
	    	LPccButton button = null;
				...
				switch(eventType) {
					...
					case XmlPullParser.START_TAG:
						...
						// if <checkbox>
							...
							if (pane != null)
							{
							   	checkbox = new LPccCheckBox(this);
	...
	{
		String exitInActions = Utils.executeRootCommandInThread("grep exit /res/lpowercc/actions/*");
		boolean optimized = false;
		if(exitInActions == null || exitInActions.length() == 0)
        {
			Utils.executeRootCommandInThread("source /res/lpowercc/lpowercc-helper");
	        Utils.executeRootCommandInThread("read_defaults");
	        Utils.executeRootCommandInThread("read_config");
	        optimized = true;
        }
       	for (int i = 0; i < lpccTabList.size(); i++)
    	{
   	   		LPccTab tab = lpccTabList.get(i);
   	   		for  (int j = 0; j < tab.panes.size(); j++)
	        {
	        	LPccPane pane = tab.panes.get(j);
	        	for  (int k = 0; k < pane.controls.size(); k++)
	        	{
	        		LPccControl control = pane.controls.get(k);
	...
	private boolean applyUserSelections() //no UI access
	{
		...
		for (int i = 0; i < lpccTabList.size(); i++)
    	{
	        ...
	        for  (int j = 0; j < tab.panes.size(); j++)
	        {
	        	...
	        	{
	        		LPccControl control = pane.controls.get(k);
	        		if(control.isChanged() && control.canGetValueFromScript) //TODO: Move these checks into the LPccControl class
	        		{
	        			Log.i(LOG_TAG, "Control cambiado:" + control.name);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        else
        {
         	if (Utils.canRunRootCommandsInThread())
         	{
         		...
         		dialog = ProgressDialog.show(this, getResources().getText(R.string.app_name), "Cargando! Por favor espere...", true);
         	}
         	else
         	{
...
//    	handler.postDelayed(r, 5000);   	
	...
	private void createSwipableTabs(final ActionBar actionBar) {
        ...
        mViewPager.setOffscreenPageLimit(lpccTabList.size());
    ...
    public class SectionsPagerAdapter extends FragmentPagerAdapter {
        ...
        public Fragment getItem(int i) {
            Fragment fragment = new LPccTabFragment();
            fragment.setRetainInstance(true);
            Bundle args = new Bundle();
            args.putInt(LPccTabFragment.ARG_SECTION_NUMBER, i);
        ...
        @Override
        public int getCount() {
        	return lpccTabList.size();
        }

        @Override
        public CharSequence getPageTitle(int position) {
        	if (position < lpccTabList.size())
        	{
        		return lpccTabList.get(position).name;
        	}
            return null;
        }
    }
     ...
     */
    public static class LPccTabFragment extends Fragment {
        public LPccTabFragment() {
        }
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container,
    		...
    		ScrollView tabEnclosingLayout = createLPccTab(mTabIndex);
	        
	        return tabEnclosingLayout;
        }

		private ScrollView createLPccTab(Integer tabIndex) {
			ScrollView tabEnclosingLayout = new ScrollView(getActivity());
       		...
       		LPccTab tab = lpccTabList.get(tabIndex);
    ...
    private class LoadDynamicUI extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) 
        {
        	if (testingWithNoKernelSupport)
        	{
        		...
        		Log.e(LOG_TAG, "NumOfTabs:" + lpccTabList.size());
        	}  
